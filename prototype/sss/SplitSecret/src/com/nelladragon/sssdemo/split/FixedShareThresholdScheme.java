/* $Id: ShamirSecretSharing.java,v 1.9 2012/06/29 05:54:07 weyerd1 Exp $ */
/*
 * Copyright (c) 1997-2014 EMC Corporation. All rights reserved.
 *
 * EMC, RSA, the RSA Logo, and BSAFE are either registered trademarks or
...
public final class FixedShareThresholdScheme {
     ...
     * key secret and the password share values fixed.
     *
    ...
    public static BigInteger[] generateProactivizationCoefficients(
        ...
        // For proactivization we are keeping the key and the password share static.
        // That is, for the x values corresponding to the password share and the
        // key value, the y value should be 0.

        // Imagine: y = a*x**3 + b*x**2 + c*x + d
        // For key, x=0, we want y=0, hence d=0.
    ...
    /**
     * Generate coefficients to generate a random polynomial, but keep the
     * key secret value fixed and change the password by a known amount.
     *
     * @param p Prime modulus.
     * @param threshold The number of shares needed to recover the secret.
     * @param random A PRNG.
    ...
    public static BigInteger[] generatePasswordChangeCoefficients(
        ...
        // For password change we are keeping the key static,
        // and changing the password by the passwordChangeValue.
        // That is: for the x value corresponding to key value, the y value should be 0.
    ...
    /**
     * Randomly generate the secret value (the key) and the Y values for all X values,
    ...
    /**
     * Generate a single random X value mod p. The X value can not be 0 (maps to the secret key
    ...
    public static BigInteger generateXval(BigInteger p, BigInteger[] existingXValues, int numXvals, SecureRandom random) {
        ...
        do {
            ...
            // X values can not be zero (the secret key value), or one (the fixed password share value)
