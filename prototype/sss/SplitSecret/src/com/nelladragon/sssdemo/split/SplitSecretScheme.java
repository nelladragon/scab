...
public class SplitSecretScheme {
    ...
    public static final int DEFAULT_NUM_SHARES = 3;

    public SplitSecretScheme() throws Exception {
        this.random = SecureRandom.getInstance("HMACDRBG256", JCE);

    }

    /**
     * Generate a random key and random shares given a fixed password share.
     *
    ...
    public SplitKey generate(SecretShare passwordShare) throws Exception {
        ...
        SecretShare deviceShare = new SecretShare(ShareType.DEVICE, yValsAndSecret[1], xValues[0]);
        SecretShare[] externalShares = new SecretShare[numRandomShares - 1];
        for (int i = 0; i < numRandomShares-1; i++) {
            externalShares[i] = new SecretShare(ShareType.EXTERNAL, yValsAndSecret[i+2], xValues[i+1]);
        }
    ...
    /**
     * Recover the key based on shares.
     *
     * @param shares Shares to use to recover the key.
    ...
    public SecretShare addShare(SecretShare[] shares) throws Exception {
        ...
        return new SecretShare(ShareType.EXTERNAL, yValue, newXvalue);
    }


    /**
     * Proactize some shares.
     *
     * @param shares
     * @return
     * @throws Exception
    ...
    public SecretShare[] proactivizeShares(SecretShare[] shares) throws Exception {
        ...
        for (int i=0; i < shares.length; i++) {
            ...
            shares[i] = new SecretShare(shares[i].getType(), newY, shares[i].getShareX());
        }
     ...
     * The key secret stays unchanged.
     *
     * @param shares
     * @return
     * @throws Exception
